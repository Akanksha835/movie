...
public class MovieH2Service implements MovieRepository {
    ...
    public Movie addMovie(Movie movie) {
        db.update("insert INTO MOVIELIST(movieName,leadActor)values(?,?)", new MovieRowMapper(), movie.getMovieName(),
                movie.getLeadActor());
    ...
    public Movie updateMovie(int movieId, Movie movie) {
        if (movie.getMovieName() != null) {
            db.update("update MOVIELIST set movieName=?", movie.getMovieName(), movieId);
        }
        if (movie.getLeadActor() != null) {
            db.update("update MOVIELIST set leadActor=?", movie.getLeadActor(), movieId);
        }
